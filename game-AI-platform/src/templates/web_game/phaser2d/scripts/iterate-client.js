/**
 * iterate-client.js
 * Action replay + screenshot script.
 * Usage: node scripts/iterate-client.js [--actions=scripts/example-actions.json] [--port=4173]
 */
...
async function run() {
  ...
  for (const action of actions) {
    ...
    try {
      ...
      let screenshot = null;
      try {
        screenshot = await page.screenshot({ encoding: 'base64', type: 'png' });
      } catch (_) {}
